...
class BaseNetTrs(torch.nn.Module):
    def __init__(self, in_dim, out_dim, c0, dropout, ks, ds, momentum):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim

        # Transformer: d_model must match the input dimension
        self.transformer = torch.nn.Transformer(
            d_model=in_dim,  # Feature dimension
        ...
        # Final linear layer to match output dimension
        self.fc_out = torch.nn.Linear(in_dim, out_dim)
    ...
    def forward(self, us):
        ...
        """
        # Normalize inputs
        u = self.norm(us)  # (batch_size, seq_length, in_dim)

        # Transpose to match Transformer input: (seq_length, batch_size, in_dim)
        u = u.transpose(0, 1)

        # Apply Transformer
        y = self.transformer(u, u)  # (seq_length, batch_size, in_dim)

        # Transpose back to (batch_size, seq_length, in_dim)
        y = y.transpose(0, 1)

        # Pass through final linear layer
        y = self.fc_out(y)  # (batch_size, seq_length, out_dim)
        return y
